...
Pages.rkbKapaHK = async function(){
  const root = U.qs('#app-root');
  const ok = await STORE.ensureActuals(['rkb','rkb_items','ydivisi','yestate','yplant']);
  if(!ok){ U.showPullRequiredModal('Data RKB belum tersedia. Silakan tarik Master & Data Aktual dulu.', ()=>location.hash='#/'); return; }

  const me   = SESSION.profile() || {};
  const role = String(me.role||'').toLowerCase();

  const rkbHdrAll   = STORE.getActual('rkb') || [];
  const rkbItemsAll = STORE.getActual('rkb_items') || [];
  const divMap      = new Map((STORE.getMaster('ydivisi')||[]).map(d=>[String(d.id), d]));
  // scope sesuai role
  let rkbHdr = [];
  if (role === 'admin') {
    rkbHdr = rkbHdrAll.slice();
  } else if (role === 'asisten') {
    rkbHdr = rkbHdrAll.filter(x => String(x.divisi_id||'') === String(me.divisi_id||''));
  } else {
    rkbHdr = rkbHdrAll.filter(x => String(x.estate_id||'') === String(me.estate_id||''));
  }

  const hdrByNomor   = new Map(rkbHdr.map(h=>[String(h.nomor), h]));
  const allowedNomor = new Set(rkbHdr.map(h=> String(h.nomor)));

  // agregasi item per nomor
  const agg = new Map();
  (rkbItemsAll||[]).forEach(it=>{
  ...
  agg.forEach((a, nomor)=>{
    ...
    const hk_total= Math.round(a.hk_total||0);
    const tk_bhl  = Math.ceil(hk_bhl/20);
    const tk_sku  = Math.ceil(hk_sku/25);
    rows.push({
      nomor,
      periode: U.fmt.periodeYM(h.periode||''),
      divisi : String(h.divisi||'') || (divMap.get(String(h.divisi_id||''))?.nama || ''),
      hk_bhl, hk_sku, hk_total,
      tk_bhl, tk_sku, tk_total: tk_bhl+tk_sku,
  ...
  const periodeOptions = Array.from(new Set(rkbHdr.map(h=>U.fmt.periodeYM(h.periode||'')))).sort().reverse();
  ...
  function applyFilter(){
    ...
    if(qq){
      arr = arr.filter(r=> [r.nomor, r.periode, r.divisi, r.status]
        .some(v=> String(v||'').toLowerCase().includes(qq)));
    }
    arr.sort((a,b)=> (a.periode===b.periode)
  ...
  // --- API helper (gunakan helper proyek bila ada, fallback ke fetch JSON) ---
    ...
    const base =
        ...
        (window.CONFIG && CONFIG.gsUrl) || ''; // fallback Apps Script URL
  ...
  function render(){
    ...
    root.innerHTML = `
      <div class="card shadow-sm"><div class="card-body">
        ...
        <h5 class="mb-0">Kapasitas HK – RKB</h5>
        <div class="btn-group">
        ...
        <div class="row g-2 mb-3">
          ...
          <div class="col-12 col-md-5">
            ...
            <input id="f-q" class="form-control" placeholder="Cari nomor/divisi/status..." value="${q}">
          </div>
        ...
        <div class="table-responsive">
          <table class="table table-sm table-hover align-middle">
            <thead class="table-light">
              <tr>
                <th>No</th><th>Nomor RKB</th><th>Periode</th><th>Divisi</th>
                <th class="text-end">HK BHL</th><th class="text-end">HK SKU</th><th class="text-end">HK (Total)</th>
            ...
            <tbody>
              ${ slice.map((r,i)=>`
                <tr>
                  ...
                  <td>${EH(r.periode)}</td>
                  <td>${EH(r.divisi)}</td>
                  <td class="text-end">${U.fmt.id0(r.hk_bhl)}</td>
                  <td class="text-end">${U.fmt.id0(r.hk_sku)}</td>
                  <td class="text-end">${U.fmt.id0(r.hk_total)}</td>
    ...
    try{
        // Jika periode kosong, konfirmasi rebuild semua
        if(!periodeFilter){
        ...
        kind: 'rkb',
        periode: periodeFilter || ''   // '' = semua periode
        });
        const written = resp?.written ?? resp?.data?.length ?? 0;
        U.toast(`Berhasil rebuild Kapasitas RKB ke Sheet (${written} baris).`, 'success');
    }catch(err){
        console.error(err);
  ...
  function exportExcel(arr){
    ...
    const header = ['No','Nomor RKB','Periode','Divisi','HK BHL','HK SKU','HK (Total)','TK BHL','TK SKU','TK (Total)','Status'];
    const rows   = arr.map((r,i)=>[ i+1, r.nomor, r.periode, r.divisi, r.hk_bhl, r.hk_sku, r.hk_total, r.tk_bhl, r.tk_sku, r.tk_total, r.status ]);
    const ws     = XLSX.utils.aoa_to_sheet([header, ...rows]);
    const wb     = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Kapasitas RKB');
    XLSX.writeFile(wb, 'kapa_hk_rkb.xlsx');
  }
